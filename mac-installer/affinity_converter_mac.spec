...
_hidden = (
    collect_submodules("marker") + collect_submodules("surya") +
    collect_submodules("pdftext") + collect_submodules("google.genai") +
    collect_submodules("google.auth") + collect_submodules("google.api_core") +
...
_datas = (
    collect_data_files("marker") + collect_data_files("surya") +
    collect_data_files("pdftext") + collect_data_files("pypdfium2") +
...
a = Analysis(["main.py"], pathex=[str(SPEC_DIR), str(MARKER_PDF)],
    binaries=[], datas=_datas, hiddenimports=_hidden,
    hookspath=[], hooksconfig={}, runtime_hooks=[],
    excludes=["fastapi", "uvicorn", "google.cloud.storage"], noarchive=False)
pyz = PYZ(a.pure)
exe = EXE(pyz, a.scripts, [], exclude_binaries=True,
    name="Affinity-PDF-Markdown Converter", debug=False,
    bootloader_ignore_signals=False, strip=False, upx=False,
    console=False, disable_windowed_traceback=False)
coll = COLLECT(exe, a.binaries, a.datas, strip=False, upx=False,
    name="Affinity-PDF-Markdown Converter")
app = BUNDLE(coll, name="Affinity-PDF-Markdown Converter.app",
    bundle_identifier="com.noblecollective.affinity-converter",
    info_plist={"CFBundleName": "Affinity-PDF-Markdown Converter",
