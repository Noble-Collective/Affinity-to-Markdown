...
def _load_styles_yaml(yaml_path: Path) -> tuple[dict, dict, str]:
    """
    Parse styles.yaml. Returns (style_map, name_map, fallback).

    style_map: { style_id (int) -> {"name": str, "markdown": str} }
      Entries that have an explicit numeric id: field.

    name_map: { style_name (str) -> markdown_role (str) }
      Entries that have a name: field but NO id: field.  These are resolved
      to numeric IDs at runtime by scanning the .afpub binary for each file.
      This allows the same styles.yaml to work across files that share the
      same Affinity template even when the numeric IDs differ between files.

    fallback: "warn" | "skip" | "body"
    """
    ...
    if not yaml_path.exists():
        ...
        return style_map, name_map, fallback
    ...
    for raw_line in lines:
        ...
        if line.startswith("- id:"):
            if current:
                if "id" in current and "markdown" in current:
                    style_map[current["id"]] = current
                elif "name" in current and "markdown" in current and "id" not in current:
                    name_map[current["name"]] = current["markdown"]
...
def _resolve_names_to_ids(
    ...
    for sp_start, sp_end in bounds:
        ...
        if sum(len(t) for _, t, _, _ in blocks) > 100_000:
            continue
        for _, text, char_runs, _ in blocks:
            prev = 0
            for ce, csid in char_runs:
                seg = text[prev:ce].replace(_LINE_SEP, " ").strip().rstrip("\x00")
                if seg and _is_content(seg):
                    char_freq[csid] = char_freq.get(csid, 0) + 1
                    if csid not in char_samples:
                        char_samples[csid] = []
                    if len(char_samples[csid]) < 3:
                        char_samples[csid].append(seg[:60])
                prev = ce
    ...
    _ROLE_PRIORITY = {
        ...
        "bold": 9, "italic": 10, "p_italic": 11, "1.": 12, "-": 13, "<<": 14,
        "": 15, "SKIP": 99,
    }
    sorted_names = sorted(
        name_map.items(),
        key=lambda kv: _ROLE_PRIORITY.get(kv[1], 50)
    )
    for name, role in sorted_names:
        if name in auto:
            run_id = auto[name]
            if run_id not in claimed:
                resolved[run_id] = {"markdown": role, "name": name}
                claimed.add(run_id)
    ...
    if body_names:
        for cid, _ in sorted(char_freq.items(), key=lambda x: -x[1]):
            if cid not in claimed:
                resolved[cid] = {"markdown": "", "name": body_names[0]}
                claimed.add(cid)
                break
    ...
    if pitalic_names:
        for cid, _ in sorted(char_freq.items(), key=lambda x: -x[1]):
            ...
            if samples and any(len(s) > 30 for s in samples):
                resolved[cid] = {"markdown": "p_italic", "name": pitalic_names[0]}
                claimed.add(cid)
                break
...
def _analyze_styles(
    ...
    if name_map:
        ...
        style_map = {**style_map, **new_entries}
...
STEP 2 - Assign markdown roles:
  ...
  Paragraph styles (Section 3):
    ...
    ">"    blockquote (overrides body-text character style for scripture passages)
...
def _convert(
    ...
    if name_map:
        ...
        if name_resolved:
            new_entries = {rid: e for rid, e in name_resolved.items()
                           if rid not in style_map}
            n_new = len(new_entries)
            print(f"  Resolved      {len(name_resolved)} named styles ({n_new} new IDs)")
            style_map = {**style_map, **new_entries}
...
def main() -> None:
    ...
    print(f"Loaded {len(style_map)} id-keyed + {len(name_map)} name-keyed styles  (fallback: {fallback!r})")
    ...
    for i, afpub_path in enumerate(input_paths):
        ...
        try:
            if dump_mode:
                ...
                _analyze_styles(afpub_path, style_map, name_map, zstd_lib)
            else:
                output_path = (explicit_output
                               if explicit_output is not None
                               else afpub_path.with_suffix(".md"))
                _convert(afpub_path, output_path, style_map, name_map, fallback, zstd_lib)
